...
#print(os.path.dirname(os.path.abspath(__file__))+'/memory.txt')
app= QApplication(sys.argv)
main_win= Application()
app.exec_()
# Я смог настроить так чтобы доступ к файлу было при помощи библиотеки OS и из-за этого можно может быть оно будет работать на всех устройствах
# У меня проблемы с таймером
# Также при изменении теста через редактор обновления теста приходят только при втором заходе на тэст с редактора я пока не знаю как это изменить