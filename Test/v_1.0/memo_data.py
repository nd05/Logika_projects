...
class Form():
    # создание собственного класса для хранения данных каждого вопроса
...
class edit_memory(QWidget):
    ...
    def __init__(self, parent):
        # конструктор класса edit_memory
    ...
    def change_func(self, modelIndex): 
        # функция для изменения вопроса и его ответов и сохранения изменений
        if self.active != -1:
    ...
    def add_qw(self):
        # функция добавления вопроса в тест
    ...
    def del_qw(self):
        ...
        if self.active != -1:
            self.qwestions.remove(self.qwestions[self.active])
            self.qList = []
            for i in range(len(self.qwestions)):
                self.qList.append(self.qwestions[i].question)
            self.slm.setStringList(self.qList)
            ...
            self.active = -1
    def go_to_test(self):
        # функция перехода к тестированию
        self.write_data()
        self.parent.show()        
        self.hide() 